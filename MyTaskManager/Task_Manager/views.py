from .forms import CreateTaskForm, CreateNewTaskGroupForm
...
class TaskDitail(mixins.LoginRequiredMixin, View):
    ...
    form_class = CreateTaskForm
    BookFormSet = modelformset_factory(Task, StagesOfExecuting, exclude=('task',), extra=4)

    def get(self, request, *args, **kwargs):
        user = request.user.pk
        task = get_object_or_404(Task, pk=kwargs['pk'])
        formset = self.BookFormSet(instance=task)
        context = {
            'title': 'Деталь',
    ...
    def post(self, request, *args, **kwargs):
        form = self.form_class(request.POST, request.FILES)
        formset = self.BookFormSet(request.POST, request.FILES)
